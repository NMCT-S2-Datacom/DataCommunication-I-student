...
class Thermostat:
    ...
    def update(self):
        delta = self.sensor.get_temperature() - self.set_point
        color_delta = max(min(delta / self.hysteresis / 2, 1), -1) * 127
        self.led.set_color(127 + color_delta, 0, 127 - color_delta)

        if delta > self.hysteresis / 2:
            print('Too hot, fan on!')
            self.heat.off()
            self.fan.on()
        elif delta < -self.hysteresis / 2:
